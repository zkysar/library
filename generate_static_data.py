...
def parse_date(date_str):
    ...
    try:
        ...
        log.warning(
            f"Could not parse date: {date_str} (cleaned: {cleaned_date})")
        return None
...
def generate_events_json():
    """Generate static JSON file from CSV data"""
    ...
    text_columns = ['event_title', 'event_description',
                    'event_link', 'library_name', 'library_address']
    df[text_columns] = df[text_columns].fillna('')

    # Group recurring events
    df['recurring'] = df.duplicated(
        subset=['event_title', 'library_url'], keep=False)
    ...
    for _, row in df.iterrows():
        try:
            ...
            date = parse_date(row['event_date']) if pd.notna(
                row['event_date']) else None
    ...
    log.info(f"Generated events.json with {len(calendar_events)} events")
